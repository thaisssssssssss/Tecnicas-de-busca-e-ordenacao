...
BST* insereRandomicamenteBST(BST* bst, int chave, int valor){
    if(bst == NULL) return criaNo(chave, valor);
    int r = rand() % (bst->tamanho + 1);

    if(!r) return insereChaveBST(bst, chave, valor);

    int comp = compara(chave, bst->chave);
    if(comp < 0) bst->esq = insereRandomicamenteBST(bst->esq, chave, valor);
    else if(comp > 0) bst->dir = insereRandomicamenteBST(bst->dir, chave, valor);
    else bst->valor = valor;
